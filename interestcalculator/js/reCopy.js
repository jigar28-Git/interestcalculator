	...
	$.fn.relCopy = function(options) {
		...
		this.each(function() {
			...
			$(this).click(function(){
				...
				if (settings.clearInputs){
					$(clone).find(':input').each(function(){
						...
						switch(type)
						{
							...
							if(!$(this).hasClass('deli_date mws-textinput hasDatepicker'))
							{
						  		$(this).val("");
							}						
				...
				$(document).find($(".deli_date")).each(function(index,element){					
					$(this).datepicker("destroy");
					$(this).removeClass('hasDatepicker');
					$(this).datepicker({dateFormat:'dd-mm-yy',changeMonth: true,changeYear: true});
					var picker = "deli_date"+(counter+1);
					if(counter == 1){counter='';}
					var dtval  = $('#deli_date'+counter).val();
					$('#'+picker).val(dtval);
				});				
				...
				$(clone).find('select').each(function() {
					if ($(this).hasClass('mws-select')) 
					{
						...
						setTimeout(function () 
						{
							that.removeData('select2').select2();				
							//that.focus();
						}, 0);
					}
                });
